...
class Student < ActiveRecord::Base
  ...
  validates_presence_of :admission_no, :admission_date, :first_name, :date_of_birth
  ...
  def method_missing(method_sym, *arguments, &block)
    s = StudentAdditionalField.find_by_name method_sym.to_s.capitalize
    if s
      a = StudentAdditionalDetails.find(:last, :conditions =>{:additional_field_id => s.id, :student_id => self.id})
      if a
        a.additional_info
      else
        ''
  ...
  def create_user_and_validate
    self.email ||="ronomeya" + self.admission_no.to_s + "@gmail.com"
    if self.new_record?
      ...
      user_record.email = self.email.blank? ? "noreply#{self.admission_no.to_s}@gmail.com" : self.email.to_s
    ...
    self.email = "ronomeya#{self.admission_no}@gmail.com" if self.email.blank?
    return false unless errors.blank?
  end
  ...
  def subjects
    general_subjects = Subject.find_all_by_batch_id(self.batch.id, :conditions=>"elective_group_id IS NULL AND is_deleted=false")
      electives = StudentsSubject.find_all_by_student_id(self.id,:conditions=>"batch_id = #{self.batch.id}")
      student_electives = []
 ...
 def current_class_mean
    arr = Array.new
    self.batch.students.each do |std|
      arr << std.total_marks
    end
    arry = 0
    arr_size = arr.length
