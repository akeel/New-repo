...
class ExamGroup < ActiveRecord::Base
  ...
  def subject_wise_class_average_marks(subject,students,type,batch)
    total_student_subject_marks = 0
    students.each do |student|
      subject_total_marks = ExamScore.new
      s_t_m = subject_total_marks.grouped_exam_subject_total(subject,student,type,batch)
      total_student_subject_marks = total_student_subject_marks +  s_t_m
    end 
    
    if students.count > 0
      subject_wise_batch_average_marks = total_student_subject_marks/students.count
    else
      subject_wise_batch_average_marks = 0
    end
    return subject_wise_batch_average_marks.to_f
  end
