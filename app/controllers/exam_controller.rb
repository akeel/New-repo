...
#    filter_access_to :all
    ...
    def generated_report
        ...
        if params[:student].nil?
            ...
            student_electives = StudentsSubject.find_all_by_student_id(@student.id,:conditions=>"batch_id = #{@batch.id}") if !@student.nil?
            elective_subjects = []

             if !student_electives.nil?
            ...
            if !@student.nil?
               ...
               @graph = open_flash_chart_object(770, 350,
                "/exam/graph_for_generated_report?batch=#{@batch.id}&examgroup=#{@exam_group.id}&student=#{}")
            end
        else
...
#        end
    ...
    def generated_report5
	    ...
	    @subjects = @batch.subjects
            ...
            flash[:notice] = "select a class to continue"
            redirect_to :action=>'class_wise_report'
        end
    end
...
#        end
    ...
    #Report Card
=begin    
    def generate_report_card4
			@student = Student.find(params[:student])
			@subjects = Subject.find(:all, :conditions => {:batch_id => @student.batch_id})
			@exam_groups = ExamGroup.find_all_by_batch_id(@student.batch_id)
			puts @exam_groups.inspect
			respond_to do |format|
...
=end
        ...
        if params[:student].nil?
            ...
            general_subjects = Subject.find_all_by_batch_id(@student.batch.id, :conditions=>"elective_group_id IS NULL AND is_deleted=false")
            electives = StudentsSubject.find_all_by_student_id(@student.id,:conditions=>"batch_id = #{@student.batch.id}")
           	student_electives = []
           	electives.each do |elect|
           		elect_subject = Subject.find(elect.subject_id)
           		student_electives << elect_subject
           	end
...
#            @subjects = Subject.find(:all, :conditions => {:batch_id => @student.batch_id})
    ...
    def list_ranks_by_course
    @students = Student.find_all_by_batch_id(params[:batch_id], :order => 'first_name ASC')
    @batch = Batch.find(params[:batch_id]) 
    render(:update) { |page| page.replace_html 'students', :partial => 'ranks_by_course' }
  end
            
   
    def form_top
    	@courses = Course.find(:all, :select => 'DISTINCT course_name')
    end
    
    def list_ranks_by_form
    	@courses = Course.find_all_by_course_name(params[:course_name])
    	@course = @courses.first
    	render(:update) { |page| page.replace_html 'students', :partial => 'ranks_by_form' }
    ...
    def list_ranks_by_class_form
    	@courses = Course.find_all_by_course_name(params[:course_name])
    	@course = @courses.first
    	render(:update) { |page| page.replace_html 'students', :partial => 'ranks_by_class_form' }
    end
   ...
   #  render :pdf => 'examam',
    ...
    def update_batch_per_user_report
      @batch = Batch.find_all_by_course_id(params[:course_name], :conditions => {:is_deleted => false, :is_active => true})

      render(:update) do |page|
        page.replace_html 'update_batch', :partial=>'update_batch_per_user_report'
      end
     end
      
      
      def top_batch_students
      @course = Course.find(params[:course_name])
      	render(:update) do |page|
        	page.replace_html 'update_batch', :partial=>'top_batch_students'
      	end
      end
      
      
      def top_class_report
      @batch = Batch.find_all_by_course_id(params[:course_name], :conditions => {:is_deleted => false, :is_active => true})

      render(:update) do |page|
        page.replace_html 'update_batch', :partial=>'top_class_report'
      end
     end
     
     def best_class_in_form
      @course = Course.find(params[:course_name])
      	render(:update) do |page|
        	page.replace_html 'update_batch', :partial=>'top_class_in_form'
      	end
      end
